...
(() => {
  ...
  function resetRun() {
    ...
    lastDark = [-1, -1]; lastAdj = false; rings.length = 0; floats.length = 0; manganAt = 0;
  ...
  function purify(r) {
    ...
    if (purified === MANGAN) { manganAt = performance.now() - runStart; addFloat("満　願"); }
  ...
  function showGameOver(cleared) {
    ...
    if (!practice) { if (purified > saveData.titleRank) { saveData.titleRank = purified; saveData.title = titleFor(purified); } saveData.best = Math.max(saveData.best, purified); saveDirty = true; persistSave(); }
