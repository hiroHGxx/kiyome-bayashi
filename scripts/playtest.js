...
const state = (p) => p.evaluate(() => ({ floors: document.getElementById("score").textContent, best: document.getElementById("best").textContent, over: document.getElementById("overlay").classList.contains("show"), rankLine: !document.getElementById("final-rank-line").hidden, saved: localStorage.getItem("waiwai:" + (window.SAVE_KEY || "")), submits: window.__submits.length }));
...
(async () => {
  ...
  { const { browser, page, errors } = await open(base, "#autotest"); await page.waitForFunction(() => document.getElementById("overlay").classList.contains("show"), { polling: 300, timeout: 120000 }); const s = await state(page); check("自動プレイで終わりまで・送信0", s.over && +s.floors > 0 && s.submits === 0 && errors.length === 0, { ...s, errors }); await browser.close(); }
  // ② わざと終わる
